...
# la portada. Aqui el juego es de CINTA, asi que la pagina que en la serie se
# llama "el cartucho" se rotula "la cinta"; el nombre del fichero se deja como
# esta en toda la serie para no romper los enlaces cruzados.
...
# El pie va en el idioma de la pagina. La cinta no firma: en las cinco piezas no
# hay ni un credito ni unas iniciales en ASCII. El sello de la casa esta
# dibujado, no escrito: la pantalla de carga pone OMIKRON Softwarwe, con la
# errata grabada en la cinta.
PIE = {
    "es": "<em>El Descubrimiento de America</em> lo publico Gema Software / OMK Software para MSX en 1987, en cinta. Todos los derechos sobre el juego siguen siendo de sus titulares. Este trabajo es de preservacion, estudio y documentacion, y la imagen de la cinta no se distribuye.",
    "en": "<em>El Descubrimiento de America</em> was published by Gema Software / OMK Software for the MSX in 1987, on tape. All rights in the game remain with their holders. This is preservation, study and documentation work, and the tape image is not distributed.",
}
...
REPO = os.environ.get("DESCUBRIMIENTO_REPO",
                      "https://github.com/antxiko/Descubrimiento-disassembly")
...
def main(docdir, idioma="en"):
    ...
    for fn in sorted(os.listdir(docdir)):
        ...
        titulo = ((m.group(1) if m else fn[:-3])
                  + " — El Descubrimiento de America (Gema/OMK, 1987)")
